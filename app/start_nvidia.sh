...
start_metrics_worker() {
    ...
    nohup bash -c "
        cd ${PROJECT_DIR}
        python3 -m serve.metrics_worker \
            --host 0.0.0.0 \
            --port ${port} \
            --worker-address http://localhost:${port} \
            --controller-address http://localhost:${PORT_CONTROLLER} \
...
start_demo_backend() {
    ...
    nohup bash -c "
        cd ${PROJECT_DIR}
        python3 -m app.server --serve --port ${DEMO_BACKEND_PORT}
...
start_demo_frontend() {
    ...
    nohup bash -c "
        cd ${DEMO_DIR}
        python3 -m http.server ${DEMO_FRONTEND_PORT}
